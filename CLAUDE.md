...
- **ვაკანსიის დეტალების გვერდი + კომპანიის საჯარო პროფილი** (მომხმარებლის მოთხოვნით): აღმოჩნდა, რომ `GET /api/jobs/{id}` უკვე არსებობდა (Stage 11), მაგრამ frontend-ს არასდროს ჰქონდა მასზე გადამისამართება (job card-ზე დაჭერას არაფერი მოჰყვებოდა); კომპანიის საჯარო (non-"me") ნახვის endpoint საერთოდ არ არსებობდა — `CompanyEndpoints`-ის მთელი ჯგუფი `CompanyOnly` policy-ითაა დაცული. დაემატა: (1) ახალი public `GET /api/company/{id:guid}` endpoint (ცალკე `MapGroup`, მხოლოდ `RequireAuthorization()` — ნებისმიერი ავტორიზებული user-ისთვის, არა `CompanyOnly`); (2) `GET /api/jobs`-ს დაემატა არასავალდებულო `companyId` query param ფილტრისთვის (backward-compatible, არსებული callers ხელუხლებელია). Gateway routing ცვლილება არ დასჭირდა — ორივე უკვე არსებული `/api/company/{**catch-all}`/`/api/jobs/{**catch-all}` routes-ის ქვეშ ჯდება.
- ახალი frontend გვერდები: `/jobs/:id` (`JobDetailComponent`) — სრული აღწერა, ყველა ველი, საჭირო უნარები proficiency-ით, "განაცხადის გაგზავნა" (მხოლოდ Person role-ისთვის, `GET /api/applications/mine`-თან ჯვარედინი შემოწმებით reload-ის შემდეგაც სწორად აჩვენებს "გაგზავნილია" მდგომარეობას — არა მხოლოდ session-ის განმავლობაში, როგორც `job-recommendations`-ის მარტივი ვერსია); `/companies/:id` (`CompanyPublicProfileComponent`) — ლოგო/სახელი/ინდუსტრია/აღწერა + ყველა მათი გამოქვეყნებული ვაკანსია, თითო ვაკანსიაზე დაჭერით `/jobs/:id`-ზე გადასვლა. ორივე `authGuard`-ითაა დაცული (ორივე role-ს შეუძლია ნახვა), `SidebarNavComponent`-იან შელში.
- ვაკანსიის სათაური/კომპანიის სახელი გახდა დაწკაპუნებადი ბმულები `job-recommendations`-ის job card-ებზე (სათაური → `/jobs/:id`, კომპანია → `/companies/:id`); `my-applications`-სა და `company-applicants`-ზეც დაემატა ვაკანსიის სათაურის ბმული `/jobs/:id`-ზე (companyId ამ ორ გვერდზე DTO-ში არ არსებობს, ამიტომ კომპანიის ბმული მხოლოდ job-recommendations-სა და job-detail-ზეა — არ დამჭირვებია `ApplicationResponse`-ის გაფართოება ამ მცირე დავალებისთვის).
- **რეალურად შემოწმდა** (Playwright, სრულ სტეკზე): job card-ის სათაურზე დაჭერა → job-detail გვერდი სრული აღწერით/ხელფასით → იქიდან apply → კომპანიის ბმულზე დაჭერა → კომპანიის საჯარო გვერდი (აღწერა + 2 ვაკანსია) → იქიდან მეორე ვაკანსიაზე დაჭერა → სწორი job-detail გვერდი; ახალი, "ცივი" page load იმავე ვაკანსიაზე სწორად აჩვენებს "განაცხადი გაგზავნილია" (persisted state); Company role-ს apply ღილაკი საერთოდ არ უჩანს job-detail-ზე; მობილურზე (iPhone SE) ორივე ახალი გვერდი overflow-ის გარეშეა. Console errors — არცერთგან.
